CREATE MATERIALIZED VIEW VW_CONTAS_RECEBER_MES_PI REFRESH FORCE ON DEMAND START WITH TO_DATE('29-12-2020 10:08:44', 'DD-MM-YYYY HH24:MI:SS') NEXT SYSDATE + 5/24/60 AS
SELECT --DECODE(MO.SEQ_PLA_MOEDA,'     75601',(SUM(NVL(CR.VALOR_DOCUM,0))),0) AS VALOR_CREDITO_REAL,
       --DECODE(MO.SEQ_PLA_MOEDA,'      3501',(SUM(NVL(CR.VALOR_DOCUM,0))),0) AS VALOR_CREDITO_DOLAR,
       DECODE(MO.SEQ_PLA_MOEDA,'     75601',(SUM(NVL(CR.VALOR_DOCUM,0)) + SUM(NVL(CR.JUROS_DOCUMENTO,0)) - SUM(NVL(CR.DESCONTO_DOCUMENTO,0))),0) AS VALOR_CREDITO_REAL,
       DECODE(MO.SEQ_PLA_MOEDA,'      3501',(SUM(NVL(CR.VALOR_DOCUM,0)) + SUM(NVL(CR.JUROS_DOCUMENTO,0)) - SUM(NVL(CR.DESCONTO_DOCUMENTO,0))),0) AS VALOR_CREDITO_DOLAR,
  ...
  FROM GUSTAVO.CONTAS_RECEBER     CR,
 ...
 GROUP BY EXTRACT(MONTH FROM TO_DATE(CR.DATA_LCTO,'DD/MM/RRRR')), EXTRACT(YEAR FROM TO_DATE(CR.DATA_LCTO,'DD/MM/RRRR')),
       ...
       CR.COD_EMPRESA, CR.COD_SAFRA, CR.DATA_LCTO, MO.SEQ_PLA_MOEDA, CR.COD_FILIAL, CR.SEQ_PLA_RECEBER,
       (SUBSTR((EXTRACT(YEAR FROM TO_DATE(CR.DATA_LCTO,'DD/MM/RRRR'))),3,2) || LPAD(EXTRACT(MONTH FROM TO_DATE(CR.DATA_LCTO,'DD/MM/RRRR')),2,0));
