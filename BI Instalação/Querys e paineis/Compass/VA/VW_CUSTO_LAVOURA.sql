...
SELECT 
       ...
       (CASE
        ...
        (SELECT BANCO.WM_CONCAT(DISTINCT V.SEQ_PLA_VARIEDADE)
        FROM BANCO.RECEITUARIO_SEMENTE S, BANCO.UBS_VARIEDADES V
       ...
       (CASE
        WHEN TSA.SEQ_PLA_VARIEDADE IS NULL THEN
        (SELECT BANCO.WM_CONCAT(DISTINCT V.DESC_VARIEDADE)
        FROM BANCO.RECEITUARIO_SEMENTE S, BANCO.UBS_VARIEDADES V
        WHERE S.SEQ_PLA_ORDEM = OS.SEQ_PLA_ORDEM
        AND S.SEQ_PLA_VARIEDADE = V.SEQ_PLA_VARIEDADE)
        ELSE
...
FROM BANCO.ORDEM_SERVICO         OS,
     BANCO.ORDEM_SERV_TALHOES    OST,
     BANCO.VINCULA_TAREFA_GRUPO  VTG,
     BANCO.TAREFAS               TAR,
     BANCO.TALHOES_SAFRA         TS,
     BANCO.GRUPO_TAREFAS         GT,
     BANCO.FUNCIONARIOS_FAZENDA  FF,
     BANCO.FUNCIONARIOS_FAZENDA FF1,
     BANCO.TALHOES_SAFRA        TSA,
     BANCO.UBS_VARIEDADES       VAA,
     BANCO.LIGA_TALHAO_APLICACAO LTA,
     ...
     BANCO.PRODUTOS_INSUMOS     PRI,
     BANCO.VAZAO                VA,
     BANCO.FINALIDADE_ORDEM_SERVICO FN,
     (SELECT esl.seq_pla_sai_item,
               BANCO.WM_CONCAT(UL.NR_LOTE) as lote
          FROM BANCO.EST_SAIDAS_LOTES ESL, BANCO.UBS_LOTES UL
         WHERE ESL.SEQ_PLA_LOTES = UL.SEQ_PLA_LOTES
         GROUP BY ESL.SEQ_PLA_SAI_ITEM
         union
        SELECT esl.seq_pla_sai_item,
               BANCO.WM_CONCAT(pl.lote) as lote
          FROM BANCO.EST_SAIDAS_LOTES ESL, BANCO.produto_lotes pl
