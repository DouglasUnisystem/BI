CREATE OR REPLACE FUNCTION FSALDO_BANCOS (MCOD_EMPRESA CHAR:='0', MCOD_FILIAL CHAR:='0', MDATAB DATE, MDATAF DATE) RETURN NUMBER IS
  mResultado NUMBER;
BEGIN
    ...
    FROM (SELECT SUM(ROUND(ROUND(NVL(SC.SALDO_CONTA,0),2) - ROUND(NVL(MV.DEBITO_ANTERIOR,0),2) + ROUND(NVL(MV.CREDITO_ANTERIOR,0),2) - ROUND(NVL(MV.VALOR_DEBITO,0),2) + ROUND(NVL(MV.VALOR_CREDITO,0),2),2)) AS SALDO_ATUAL
            FROM AGRICOLA.CTB_PLANO_CONTAS PC,
                (SELECT CT.CTB_TABELA,
                        CT.CTB_CONTA,
                        SUM(DECODE(HI.DEB_CRED,'D',DECODE(SIGN(TO_DATE(TRUNC(FI.DATA_DOCUM),'DD/MM/YYYY')-TO_DATE(MDATAB, 'DD/MM/YYYY')),-1,ROUND(FI.VALOR_REAL,2) + ROUND(DECODE(FI.JUROS_CPAGAR,'S',0,DECODE(FI.VALOR_JUROS,NULL,0,FI.VALOR_JUROS)-DECODE(FI.VALOR_ANTECIPACAO,NULL,0,FI.VALOR_ANTECIPACAO)),2),0),0)) AS DEBITO_ANTERIOR,
                        SUM(DECODE(HI.DEB_CRED,'C',DECODE(SIGN(TO_DATE(TRUNC(FI.DATA_DOCUM),'DD/MM/YYYY')-TO_DATE(MDATAB, 'DD/MM/YYYY')),-1,ROUND(FI.VALOR_REAL,2) + ROUND(DECODE(FI.JUROS_CPAGAR,'S',0,DECODE(FI.VALOR_JUROS,NULL,0,FI.VALOR_JUROS)-DECODE(FI.VALOR_ANTECIPACAO,NULL,0,FI.VALOR_ANTECIPACAO)),2),0),0)) AS CREDITO_ANTERIOR,
                        SUM(DECODE(HI.DEB_CRED,'D',DECODE(SIGN(TO_DATE(TRUNC(FI.DATA_DOCUM),'DD/MM/YYYY')-TO_DATE(MDATAB, 'DD/MM/YYYY')),-1,0,DECODE(SIGN(TO_DATE(TRUNC(FI.DATA_DOCUM), 'DD/MM/YYYY') - TO_DATE(TRUNC(MDATAF), 'DD/MM/YYYY')),1,0,ROUND(FI.VALOR_REAL,2) + ROUND(DECODE(FI.JUROS_CPAGAR,'S',0,DECODE(FI.VALOR_JUROS,NULL,0,FI.VALOR_JUROS)-DECODE(FI.VALOR_ANTECIPACAO,NULL,0,FI.VALOR_ANTECIPACAO)),2))),0)) AS VALOR_DEBITO,
                        SUM(DECODE(HI.DEB_CRED,'C',DECODE(SIGN(TO_DATE(TRUNC(FI.DATA_DOCUM),'DD/MM/YYYY')-TO_DATE(MDATAB, 'DD/MM/YYYY')),-1,0,DECODE(SIGN(TO_DATE(TRUNC(FI.DATA_DOCUM), 'DD/MM/YYYY') - TO_DATE(TRUNC(MDATAF), 'DD/MM/YYYY')),1,0,ROUND(FI.VALOR_REAL,2) + ROUND(DECODE(FI.JUROS_CPAGAR,'S',0,DECODE(FI.VALOR_JUROS,NULL,0,FI.VALOR_JUROS)-DECODE(FI.VALOR_ANTECIPACAO,NULL,0,FI.VALOR_ANTECIPACAO)),2))),0)) AS VALOR_CREDITO
