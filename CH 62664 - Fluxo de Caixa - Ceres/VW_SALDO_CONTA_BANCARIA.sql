CREATE OR REPLACE VIEW VW_SALDO_CONTA_BANCARIA AS
SELECT 0.01 as TOTAL_PREVISTO,
       SUM(ROUND(NVL(SC.SALDO_CONTA  ,0),2)) as SALDO_CONTA,
  ...
  FROM AGRICOLA.CTB_PLANO_CONTAS PC,
       (SELECT CT.CTB_TABELA,
               ...
               SUM(DECODE(HI.DEB_CRED,'D',DECODE(SIGN(TRUNC(FI.DATA_DOCUM)-TO_DATE('01/01/2000')),-1,0,DECODE(SIGN(TRUNC(FI.DATA_DOCUM)-TO_DATE(SYSDATE)),1,0,ROUND(FI.VALOR_REAL,2)+ROUND(DECODE(FI.JUROS_CPAGAR,'S',0,DECODE(FI.VALOR_JUROS,NULL,0,FI.VALOR_JUROS)-DECODE(FI.VALOR_ANTECIPACAO,NULL,0,FI.VALOR_ANTECIPACAO)),2))),0)) AS VALOR_DEBITO,
               SUM(DECODE(HI.DEB_CRED,'C',DECODE(SIGN(TRUNC(FI.DATA_DOCUM)-TO_DATE('01/01/2000')),-1,0,DECODE(SIGN(TRUNC(FI.DATA_DOCUM)-TO_DATE(SYSDATE)),1,0,ROUND(FI.VALOR_REAL,2)+ROUND(DECODE(FI.JUROS_CPAGAR,'S',0,DECODE(FI.VALOR_JUROS,NULL,0,FI.VALOR_JUROS)-DECODE(FI.VALOR_ANTECIPACAO,NULL,0,FI.VALOR_ANTECIPACAO)),2))),0)) AS VALOR_CREDITO
         ...
         WHERE FI.SEQ_PLA_FIN_HIST   = HI.SEQ_PLA_FIN_HIST
           ...
           AND TRUNC(FI.DATA_DOCUM) <= SYSDATE
           AND CT.TIPO_CONTA = 'B'
 ...
 WHERE PC.GRAU       = '5'
   ...
   AND (PC.CTB_TABELA,PC.CTB_CONTA) IN (SELECT CTB_TABELA,CTB_CONTA
                                         ...
                                         WHERE TIPO_CONTA = 'B')
 GROUP BY MV.CTB_TABELA, MV.CTB_CONTA;
